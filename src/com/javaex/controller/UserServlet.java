		...
		if("joinform".equals(actionName)) {
	      ...
	      response.getWriter().write(dao.idCheck(email)); 
			...
			if(vo==null) {
				...
				response.sendRedirect("/mysite_jimin/user?a=loginform&result=fail");
			} else {
				System.out.println("성공");
				HttpSession session = request.getSession(true);
				session.setAttribute("authUser", vo);
				
				response.sendRedirect("/mysite_jimin/main");
				return;
			}
			...
			WebUtil.redirect(request, response, "/mysite_jimin/main");
			/*response.sendRedirect("/mysite_jimin/main");*/
		}
